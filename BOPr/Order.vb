...
Public Class Order
    ...
    Private m_FoodProvidedBy As String
    Private m_OrderReceivedFrom As String
    ...
    Friend Sub LoadData(ByVal sqlDr As SqlDataReader)
        ...
        For i = 0 To sqlDr.FieldCount - 1 'reads every column in a record
            ...
            Select Case SqlDrColName
                ...
                Case "FoodProvidedBy"
                    If sqlDr.IsDBNull(i) Then
                        m_FoodProvidedBy = String.Empty
                    Else
                        m_FoodProvidedBy = sqlDr.GetString(i)
                    End If
                Case "OrderReceivedFrom"
                    If sqlDr.IsDBNull(i) Then
                        m_OrderReceivedFrom = String.Empty
                    Else
                        m_OrderReceivedFrom = sqlDr.GetString(i)
                    End If
    ...
    Public Property FoodProvidedBy() As String
        Get
            Return m_FoodProvidedBy
        End Get
        Set(ByVal Value As String)
            m_FoodProvidedBy = Value
        End Set
    End Property
    Public Property OrderReceivedFrom() As String
        Get
            Return m_OrderReceivedFrom
        End Get
        Set(ByVal Value As String)
            m_OrderReceivedFrom = Value
        End Set
    End Property
    ...
    Public ReadOnly Property NonRevenueSale() As Double
        ...
        End Get
    End Property
    Public ReadOnly Property IsPaidByTaxExemptHouseAC() As Boolean
        Get
            'True if the order is charged to tax exempt House AC and is to be invoiced from QB
            Dim IsPaidByHouseAC As Boolean
            If (Me.PaymentAmountByMethod(Payment.enumPaymentMethod.HouseAccount) = Me.POSAmount + Me.TipAmountPaid) Then IsPaidByHouseAC = True
            Return IsPaidByHouseAC
        End Get
    End Property
    ...
    Public ReadOnly Property TaxExemptRetailBaseRevenue() As Double
        Get
            'This includes Retail Food Sale to Tax Exempt Retail Customers generally paid by PU Visa i.e. not invoiced from QB
            ' excludes NonRevenueSale items & Tips
            Dim dblOrderAmount As Double
            Dim objOrderItem As OrderItem

            'Select Case Me.IsHouseACOrder
            Select Case Me.IsPaidByTaxExemptHouseAC
                Case True
                    dblOrderAmount = 0
                Case False
        ...
        Get
            ...
            For Each objPayment In Me.AllPayments
                With objPayment
                    ...
                    If ((.Status = Payment.enumStatus.Active) AndAlso (.DEError = Payment.enumPaymentDEError.GuestCheckNotPrinted Or .DEError = Payment.enumPaymentDEError.NoError)) Then
                    ...
                    Select Case .MenuItem.ProductPricingType
                        Case Product.enumProductPricingType.SystemPricedSingleItem, Product.enumProductPricingType.UserPricedSingleItem
                            If Me.AllOrderItems.GroupOrderInProgress = False Then
                                ...
                                .CrownPrice = objMenuItem.CrownPrice
                                .Quantity = 1
                            Else
            ...
            If m_objPayments Is Nothing Then
                Select Case Me.IsPersisted
                    ...
                    Case True          'Persisted Orders
                        ...
                        '                                OrderId)
                        m_objPayments = New Payments(Payments.EnumFilter.Active, _
                                                       Payments.EnumView.CompleteView, _
                                                       OrderId)
                End Select
    ...
    Public ReadOnly Property PaymentAmountByMethod(ByVal pm As Integer) As Double
        Get
            ...
            Dim amt As Double
            For Each objPayment In Me.AllPayments
                If objPayment.PaymentMethod = pm Then
                    amt = amt + objPayment.POSAmountPaid + objPayment.TipAmountPaid
                End If
            Next
            Return amt
        End Get
    ...
    Public ReadOnly Property PosAmountByPaymentMethod(ByVal pm As Integer) As Double
        Get
            ...
            For Each objPayment In Me.AllPayments
                If objPayment.PaymentMethod = pm Then
                    amt = amt + objPayment.POSAmountPaid
                End If
            Next
            Return amt
        End Get
    ...
    Public ReadOnly Property TipAmountByPaymentMethod(ByVal pm As Integer) As Double
        Get
            ...
            For Each objPayment In Me.AllPayments
                If objPayment.PaymentMethod = pm Then
                    amt = amt + objPayment.TipAmountPaid
                End If
            Next
            Return amt
        End Get
    ...
    Public Function SetData(ByRef m_selectedSDSession As SaleDaySession) As Integer
            ...
            Select Case Me.IsPersisted      'True means update row, False means insert row in SQL table 
                Case False
                    strSqlCmd = "insert into [Order] values (" & _
                                ...
                                m_selectedSDSession.SaleDaySessionId.ToString & ",'" & _
                                Me.FoodProvidedBy & "','" & _
                                Me.OrderReceivedFrom & "')"
        ...
        Get
            Select Case DiscountType
                Case enumMGCoupon.NoDiscount
                    Return 0                'in %
                Case enumMGCoupon.HappyCurryHourDiscount
                    Return 10               'in %
                Case enumMGCoupon.SpecialDiscount1
                    Return 15               'in %
                Case enumMGCoupon.SpecialDiscount2
                    Return 20               'in %
                Case enumMGCoupon.SpecialDiscount3
                    Return 25               'in %
                Case enumMGCoupon.SpecialDiscount4
                    Return 30               'in %
                Case enumMGCoupon.SpecialDiscount5
                    Return 35               'in %
                Case enumMGCoupon.SpecialDiscount6
                    Return 40               'in %
                Case enumMGCoupon.SpecialDiscount7
                    Return 45                'in %
                Case enumMGCoupon.SpecialDiscount8
                    Return 50                'in %
            End Select
        End Get
    ...
    Public Sub SetKOTPrintedAt()
        ...
        With Me
            ...
            .AllOrderItems.UpdateKOTPrintedAt(KOTAt)
        End With
        intResultCode = Me.SetData(Nothing)
    ...
    Private Function ComposeKOT(ByVal objKOTOverride As Override) As PosDoc
        ...
        With Me
            ...
            strLine = ("Order: " & .OrderId.ToString & "/" & _
                        ...
                        " " & (Format(.EnteredAt, "MM/dd/yy hh:mm tt")))
            KOTDoc.AddHeaderLine(strLine)
            strLine = ("Printed " & (Format(SaleDays.CreateSaleDay.Now, "MM/dd/yy hh:mm tt")))

            KOTDoc.AddHeaderLine(strLine)
            If .IsDesiOrder = True Then
                KOTDoc.AddHeaderLine("________________________ D E S I _________")
            Else
                KOTDoc.AddHeaderLine("__________________________________________")
            End If
        End With
            ...
            With objOrderItem
                Select Case (.KOTPrintedAt <= DateTime.MinValue) AndAlso (.Status = OrderItem.enumStatus.Active) AndAlso _
                    ...
                    Case True
                        ...
                        Select Case .Status
                            Case OrderItem.enumStatus.Active
                                'If .OrderItemNameHindi.Length > 17 Then
                                '    strItemName = (" " & .OrderItemNameHindi.Substring(0, 17)).PadLeft(18)
                                'Else
                                '    strItemName = (" " & .OrderItemNameHindi).PadLeft(18)
                                'End If
                                If .OrderItemNameHindi.Length > 17 Then
                                    strItemName = ("   " & .OrderItemNameHindi.Substring(0, 17))
                                Else
                                    strItemName = ("   " & .OrderItemNameHindi)
                                End If
                                    ...
                                    Case False
                                        ...
                                        Select Case .UoM
                                            Case OrderItem.enumUoM.Count
                                                ...
                                                strLine = (.Quantity.ToString.PadLeft(2, " "c)) & strLine
                        ...
                        End Select
                        KOTDoc.AddDetailLine(strLine)
                        If (String.IsNullOrEmpty(.ItemMod)) OrElse (.ItemMod = " ") Then
                            'nop
                        Else
                            strLine = "   =>" & .ItemMod
                            KOTDoc.AddDetailLine(strLine)
                        ...
                        'Order Item ModNote lines
                        If (String.IsNullOrEmpty(.ItemModNote)) OrElse (.ItemModNote = " ") Then
                            'nop
                        Else
                            Dim modnotes As String() = .ItemModNote.Split(New Char() {"="c})
                            Dim modnote As String
                            For Each modnote In modnotes
                                strLine = "      " & modnote.Substring(1)
                                KOTDoc.AddDetailLine(strLine)
                            Next
                            'strLine = "=>" & .ItemModNote.Substring(0, Math.Min(18, .ItemModNote.Length))
                            'KOTDoc.AddDetailLine(strLine)
                        End If
        ...
        Select Case Me.OrderType
            Case enumOrderType.Delivery
                ...
                strLine2 = ("________DELIVERY Promised at " & Me.PromisedAt.ToShortTimeString & "________")
            Case enumOrderType.Pickup
                strLine = Me.TableSeating.ToString & " " & strCustomerName
                strLine2 = ("________PICKUP Promised at " & Me.PromisedAt.ToShortTimeString & "________")
            Case enumOrderType.EatIn
                strLine = Me.Table.TableName & "-" & Me.TableSeating.ToString
                strLine2 = "____" & Me.Waiter.EmployeeShortName & "  Promised at " & Me.PromisedAt.ToShortTimeString & "____"
        End Select
    ...
    Public ReadOnly Property GuestCheckText(ByVal checkNumber As Integer) As PosDoc
        ...
        End Get
    End Property
    Public Sub SetGuestCheckPrintedAt()
        Dim GuestCheckAt As DateTime
        Dim intResultCode As Integer
        GuestCheckAt = SaleDays.CreateSaleDay.Now
        With Me
    ...
    Private Sub ComposeGuestCheckHdr(ByVal GuestCheckDoc As PosDoc, ByVal checkNumber As Integer)
        ...
        strLine = ("E-Mail: masalagrill@msn.com").PadLeft(36)
        GuestCheckDoc.AddHeaderLine(strLine)
        'GuestCheckDoc.AddHeaderLine(strLine)
        'Select Case Me.FirstCheckPrintedAt > DateTime.MinValue
        '    Case True
        ...
        With Me
            Select Case .OrderType
                Case enumOrderType.Delivery
                    strLine = ("Delivery:" & .TakeOutCustomerName & ":" & .TakeOutCustomerPhone).PadLeft(30)
                Case enumOrderType.Pickup
    ...
    Private Sub ComposeGuestCheckDetail(ByVal GuestCheckDoc As PosDoc, ByVal checkNumber As Integer)
            ...
            With objOrderItem
                ...
                Select Case (.Status = OrderItem.enumStatus.Active) AndAlso _
                    ...
                    Case True
                        Select Case Me.AllOrderItems.IsItemAComboSubItem(objOrderItem)
                            Case True
                                strLine = ("   " + objOrderItem.OrderItemName.PadLeft(17, " "c))
                            Case False
                                'strLine = (objOrderItem.OrderItemNameHindi.PadLeft((objOrderItem.OrderItemNameHindi.Length + 3), " "c))
                                strLine = (Microsoft.VisualBasic.Left(objOrderItem.OrderItemName, 26)).PadLeft(27, " "c)
        ...
        With Me
            ...
            GuestCheckDoc.AddDetailLine(("_________").PadLeft(40))

            strLine = "TOTAL FOOD ".PadLeft(26, " "c) & (Format(.OrderBaseAmount(checkNumber), "C")).PadLeft(7, " "c)
            GuestCheckDoc.AddDetailLine(strLine.PadLeft(40))

            If .DiscountAmount(checkNumber) < 0 Then
                'strLine = ("Less:" & [Enum].GetName(GetType(Order.enumMGCoupon), .DiscountType)).PadLeft(20, " "c) & _
                strLine = ("==> LESS: Discount " & CStr(.DiscountType)).PadLeft(26, " "c) & _
                            (Format(.DiscountAmount(checkNumber), "C")).PadLeft(7, " "c)
                GuestCheckDoc.AddDetailLine((strLine).PadLeft(40))

                GuestCheckDoc.AddDetailLine(("_________").PadLeft(40))


                strLine = ("Net").PadLeft(26, " "c) & _
                             (Format(.OrderBaseAmount(checkNumber) + .DiscountAmount(checkNumber), "C")).PadLeft(7, " "c)
                GuestCheckDoc.AddDetailLine((strLine).PadLeft(40))
            End If

            strLine = ("NJ Sales Tax ").PadLeft(26, " "c) & _
                             (Format(.OrderSalesTaxAmount(checkNumber), "C")).PadLeft(7, " "c)
            GuestCheckDoc.AddDetailLine((strLine).PadLeft(40))

            GuestCheckDoc.AddDetailLine(("_________").PadLeft(40))

            strLine = ("Total Charge ").PadLeft(26, " "c) & _
                             (Format(.POSAmount(checkNumber), "C")).PadLeft(7, " "c)
            GuestCheckDoc.AddDetailLine((strLine).PadLeft(40))

            GuestCheckDoc.AddDetailLine(("******************************************").PadLeft(37))
        End With
    End Sub
    ...
    Private Sub ComposeGuestCheckFooter(ByVal GuestCheckDoc As PosDoc, ByVal checkNumber As Integer)
        ...
        'Check Total Summary
        With Me
            strLine = ("Total").PadLeft(12, " "c) & _
                             (Format(.POSAmount(checkNumber), "C")).PadLeft(8, " "c)
            GuestCheckDoc.AddFooterLine(strLine)

            If .TipType = enumTips.NoTipsAdded Then
                strLine = ("+  Tip").PadLeft(12, " "c)
                GuestCheckDoc.AddFooterLine(strLine)
                GuestCheckDoc.AddFooterLine(("_____").PadLeft(21))
                strLine = ("Pay").PadLeft(12, " "c)
                GuestCheckDoc.AddFooterLine(strLine)
                strLine = .Table.TableName & "-" & .TableSeating.ToString
                GuestCheckDoc.AddFooterLine(strLine)
            Else
                strLine = ("Tip Added").PadLeft(12, " "c) & _
                             (Format(.TipAmount(checkNumber), "C")).PadLeft(8, " "c)
                GuestCheckDoc.AddFooterLine(strLine)
                GuestCheckDoc.AddFooterLine(("______").PadLeft(20))
                strLine = ("Pay").PadLeft(12, " "c) & _
            ...
            End If
        End With
        If Me.OrderType = enumOrderType.Delivery Then
            GuestCheckDoc.AddFooterLine("Deliver To:")
            GuestCheckDoc.AddFooterLine(Me.TakeOutCustomerName)
            If String.IsNullOrEmpty(Me.TakeOutCustomerPhone) Then
                'nop
            Else
                GuestCheckDoc.AddFooterLine(Me.TakeOutCustomerPhone)
            End If
            If String.IsNullOrEmpty(Me.TakeOutCustomerAddress) Then
                'nop
            Else
                GuestCheckDoc.AddFooterLine(Me.TakeOutCustomerAddress)
            End If
        ...
        If Me.IsHouseACOrder Then
            ...
            If strLine.Length > 21 Then strLine = strLine.Substring(0, 21)
            'If strLine2.Length > 21 Then strLine2 = strLine2.Substring(0, 21)
            ...
            Select Case Me.Customer.CustomerType
                Case Customer.enumCustomerType.CorpAccount, Customer.enumCustomerType.PUAccount
                    ...
                    GuestCheckDoc.AddFooterLine(strLine)
                    GuestCheckDoc.AddFooterLine(" ")        'add a blank line
                    GuestCheckDoc.AddFooterLine("Name:") '.PadLeft(20))
                    GuestCheckDoc.AddFooterLine(" ")
    ...
    Private Function GetCustomerMsg() As String
        ...
        Randomize()
        intMsg = CInt(Int(4 * Rnd()))  ' Generate random value between 0 and 3.
        Select Case CType(intMsg, enumCustomerMsg)
            Case enumCustomerMsg.HappyCurryHour
                strMsg = "10% off on All Take Out Orders"
            Case enumCustomerMsg.HolidayParties
                strMsg = "15-20% off on Catering orders!"
            Case enumCustomerMsg.LunchBuffet
                strMsg = "Lunch Buffet for only $10.99!"
            Case enumCustomerMsg.MGGiftCertificate
                strMsg = "We offer Gift Certificates."
        End Select
        Return strMsg
    End Function
