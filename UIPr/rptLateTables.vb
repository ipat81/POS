...
Public Class rptLateTables
    ...
    Friend Sub SetReportViewer(ByVal value As frmFGReportViewer)
        ...
        With m_frmReportviewer
            .Report = frmFGReportViewer.enumReport.LateTablesList
    ...
    Private Sub LoadOrders(ByVal objSDSession As SaleDaySession)
        ...
        For Each objOrder In objSDSession.AllOrders
            If objOrder.PaidAt >= objSDSession.SaleDaySessionTo Then
                With fgReport
                    .Item(intNextRow, .Cols.Item("SaleDate").Index) = objOrder.Saledate
                    .Item(intNextRow, .Cols.Item("SessionName").Index) = objOrder.orderSDSession.SessionName
                    .Item(intNextRow, .Cols.Item("TableSeating").Index) = objOrder.Table.TableName & "-" & objOrder.TableSeating.ToString
                    ...
                    If objOrder.OrderBaseAmount <> 0 Then
                        ...
                        .Item(intNextRow, .Cols.Item("NetBase").Index) = objOrder.OrderBaseAmount + objOrder.DiscountAmount
                    End If
                    If objOrder.TipAmountPaid <> 0 Then
                        .Item(intNextRow, .Cols.Item("Tips").Index) = objOrder.TipAmountPaid
                    End If

                    .Item(intNextRow, .Cols.Item("ClosedAt").Index) = objOrder.PaidAt
                    .Item(intNextRow, .Cols.Item("PaymentChanged").Index) = objOrder.PaymentChangedAt
    ...
    Private Sub fgReport_AfterDragColumn(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.DragRowColEventArgs) Handles fgReport.AfterDragColumn
        If e.Col <= 2 Then      'cols 3 and later are data columns
            LoadData()
        End If
    End Sub
